...
for profile in `cat profiles`;do #Iterates through different accounts in your org account. 
	...
	for region in `AWS_SHARED_CREDENTIALS_FILE=creds_dms aws ec2 describe-regions --profile $profile --r us-east-1 | jq -r ".Regions[].RegionName"`;do #Iterates through different regions in an account.
