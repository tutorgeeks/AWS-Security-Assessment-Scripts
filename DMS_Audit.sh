...
for profile in `cat profiles`;do #Iterates through different accounts in your org account. 
	...
	for region in `AWS_SHARED_CREDENTIALS_FILE=creds aws ec2 describe-regions --profile $profile --r us-east-1 | jq -r ".Regions[].RegionName"`;do #Iterates through different regions in an account.
		for rep_instance in `AWS_SHARED_CREDENTIALS_FILE=creds aws dms describe-replication-instances --region $region --profile $profile| jq -r ".ReplicationInstances[].ReplicationInstanceArn"`;do #Iterates through different replication instance
      	
	#Check if AutoMinorVersionUpgrade feature is enabled or not. 
      	#https://www.cloudconformity.com/knowledge-base/aws/DMS/auto-minor-version-upgrade.html
			AutoMinorVersionUpgrade=`AWS_SHARED_CREDENTIALS_FILE=creds aws dms describe-replication-instances --region $region --filters Name=replication-instance-arn,Values=$rep_instance --profile $profile | jq -r ".ReplicationInstances[].AutoMinorVersionUpgrade"`  		
      	
	#Check if the instance is encrypted using AWS or customer KMS key
      	#https://www.cloudconformity.com/knowledge-base/aws/DMS/encrypted-with-cmk.html
			KmsKeyId=`AWS_SHARED_CREDENTIALS_FILE=creds aws dms describe-replication-instances --region $region --filters Name=replication-instance-arn,Values=$rep_instance --profile $profile| jq -r ".ReplicationInstances[].KmsKeyId"`
